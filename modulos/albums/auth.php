  ...
  if(isset($header_login) && isset($header_password))
	{
	  if(strlen($header_login)<=2 || strlen($header_password)<=2)
		...
		{
			$result=mysql_query("select ID as num from ".$tof_users." where nickname='$header_login' and password='$header_password'");
			  ...
			  $row=mysql_fetch_array($result);
				...
				if(isset($cookie_login))
				{
				  setcookie("cookie_login");
					setcookie("cookie_login",$header_login,time()+977616000);
				}
				else
					setcookie("cookie_login",$header_login,time()+977616000);
				if(isset($cookie_password))
				{
				  setcookie("cookie_password");
					setcookie("cookie_password",$header_password,time()+977616000);
				}
				else
					setcookie("cookie_password",$header_password,time()+977616000);
			}
...
{
  if(isset($cookie_login))
	  $global_user_login=$cookie_login;
	if(isset($cookie_password))
	  $global_user_password=$cookie_password;
}
