$(function() {
		...
		function checkCaptcha(o) {
			if ( o.val()!= '{captcha_texto_session}') {
				o.addClass( "ui-state-error" );
				updateTips( "El captcha no coincide (respete may�sculas y min�sculas).");
				return false;
			} else {
				return true;
			}
		...
		$( "#dialog-form" ).dialog({
			...
			buttons: {
				"Enviar Comentario": function() {
					...
					bValid = bValid && checkRegexp( nombre, /^[a-z]([0-9a-z_])+$/i, "Nombre usuario debe ser caracteres a-z, 0-9, begin with a letter." );
